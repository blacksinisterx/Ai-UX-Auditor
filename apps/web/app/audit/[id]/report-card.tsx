...
const HOVER_COLOR: Record<HoveredBox["kind"], string> = {
  contrast: "#ef4444",
  size: "#f59e0b",
};
...
export function ReportCard({ audit }: { audit: AuditDoc }) {
  ...
  return (
    <div className="relative">
      ...
      />
      <div className="relative mx-auto flex w-full max-w-6xl flex-col gap-(--space-xl) px-(--space-lg) py-(--space-2xl)">
        <motion.div
        ...
        <div className="grid gap-(--space-xl) lg:grid-cols-2 lg:items-start">
          <motion.div
            ...
            className="lg:sticky lg:top-(--space-lg)"
          >
            <Card className="overflow-hidden p-0">
              ...
              <div className="relative">
                ...
                {hovered && naturalSize && (
                  <div
                    className="pointer-events-none absolute rounded-sm transition-all duration-150"
                    style={{
                      left: `${(hovered.box.x0 / naturalSize.width) * 100}%`,
                      top: `${(hovered.box.y0 / naturalSize.height) * 100}%`,
                      width: `${((hovered.box.x1 - hovered.box.x0) / naturalSize.width) * 100}%`,
                      height: `${((hovered.box.y1 - hovered.box.y0) / naturalSize.height) * 100}%`,
                      boxShadow: `0 0 0 2px ${HOVER_COLOR[hovered.kind]}, 0 0 16px 2px color-mix(in oklch, ${HOVER_COLOR[hovered.kind]} 70%, transparent)`,
                      backgroundColor: `color-mix(in oklch, ${HOVER_COLOR[hovered.kind]} 20%, transparent)`,
                    }}
                  />
                )}
              </div>
            ...
            <Tabs defaultValue="design-eye">
              ...
              <TabsContent value="rule-book" className="mt-(--space-md) flex flex-col gap-(--space-md)">
                <Card>
                  ...
                  <CardContent className="flex max-h-96 flex-col gap-(--space-sm) overflow-y-auto">
                    {contrastIssues.map((issue, i) => (
                      <div
                        key={i}
                        onMouseEnter={() => setHovered({ box: issue.box, kind: "contrast" })}
                        onMouseLeave={() => setHovered(null)}
                        className="cursor-default rounded-(--radius-md) px-(--space-xs) py-(--space-xs) transition-colors hover:bg-muted/50"
                      >
                ...
                <Card>
                  ...
                  <CardContent className="flex max-h-96 flex-col gap-(--space-sm) overflow-y-auto">
                    {sizeIssues.map((issue, i) => (
                      <div
                        key={i}
                        onMouseEnter={() => setHovered({ box: issue.box, kind: "size" })}
                        onMouseLeave={() => setHovered(null)}
                        className="cursor-default rounded-(--radius-md) px-(--space-xs) py-(--space-xs) transition-colors hover:bg-muted/50"
                      >
                        {i > 0 && <Separator className="mb-(--space-sm)" />}
