...
**[`docs/aura-demo-narrated.mp4`](docs/aura-demo-narrated.mp4)** — the same footage with a real voiceover and burned-in captions, generated by feeding the video above and `DEMO_SCRIPT.md` into [AI Video Narrator](https://github.com/blacksinisterx/Ai-Video-Narrator), a separate local-TTS project. Each section's narration is word-budgeted against that section's real on-screen duration, not written first and timed after — the final narrated video runs 4:59.6, matching the original almost exactly.
